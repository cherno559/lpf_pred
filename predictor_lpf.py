"""
dashboard_lpf.py — LPF 2026 Scouting Dashboard (v10.5 · Lambda Deflation)
─────────────────────────────────────────────────────────────────────────────
"""
import re, os, math
import numpy as np
...
W_XG = 0.65            
K_SHRINK = 3.0         
SCALING_FACTOR = 0.85  # NUEVO: Reduce la expectativa de gol global (LPF es Under)
DC_RHO = -0.10
MAX_GOALS_MATRIX = 7
...
def calcular_lambdas(df, eq_a, eq_b, es_loc):
    ...
    la = (l["ref_home"] if ca == "Local" else l["ref_away"]) * aa * db * SCALING_FACTOR
    lb = (l["ref_home"] if cb == "Local" else l["ref_away"]) * ab * da * SCALING_FACTOR
...
if nav == "🔮 Predictor":
    ...
    if st.button("🚀 INICIAR ANÁLISIS"):
        ...
        with t1:
            sub = sim["matrix"][:5, :5]
            st.plotly_chart(go.Figure(go.Heatmap(z=sub, x=[str(i) for i in range(5)], y=[str(i) for i in range(5)], colorscale=[[0,"#0f1829"],[1,"#e63946"]], showscale=False, text=[[f"{sub[i,j]*100:.1f}%" for j in range(5)] for i in range(5)], texttemplate="%{text}")).update_layout(**PLOT, height=350, yaxis=dict(autorange="reversed")), use_container_width=True)
        with t2:
            st.plotly_chart(fig_radar(df, ea, eb, "Local" if loc else "General", "Visitante" if loc else "General"), use_container_width=True)
        st.markdown(f'<div class="note">Deflación Aplicada: x{SCALING_FACTOR} | λ {ea}:{la} vs λ {eb}:{lb}</div>', unsafe_allow_html=True)

elif nav == "🔄 Head-to-Head":
    st.markdown('<div class="section-title">🔄 H2H Detallado</div>', unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    ea, eb = c1.selectbox("Eq A", equipos), c2.selectbox("Eq B", equipos, index=min(1, len(equipos)-1))
    ...
    st.dataframe(pd.DataFrame({f"{ea} Fav": s1["Propio"], f"{ea} Contra": s1["Concedido"], f"{eb} Fav": s2["Propio"], f"{eb} Contra": s2["Concedido"]}).dropna(), use_container_width=True)

elif nav == "📖 Perfil Rival":
    c1, c2 = st.columns(2)
    eq_p, met_p = c1.selectbox("Equipo", equipos), c2.selectbox("Métrica", metricas)
