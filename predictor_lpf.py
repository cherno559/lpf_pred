"""
dashboard_lpf.py — LPF 2026 Scouting Dashboard (v8.2 - Calibración Total)
────────────────────────────────────────────────────
Cambios:
  · Calibración Fina: Soft-Clip asimétrico a partir de 1.45 (Protege a Boca, frena a Lanús).
  · Regresión Bayesiana optimizada (K=3.8) para mayor estabilidad de mercado.
  · Restauración completa de todas las secciones visuales.
"""
...
# ── PARÁMETROS DE CALIBRACIÓN V8.2 ──
LIGA_MEDIA_GOLES = 1.17 
REGRESION_K = 3.8      # Amortiguación bayesiana
SOFT_CLIP_UMBRAL = 1.45 # Solo frena los excesos (Protege la ventaja de Boca)
SOFT_CLIP_COMPRESION = 0.32
...
# ──────────────────────────────────────────────────────────────────────
# MOTOR PREDICTOR (V8.2 CALIBRADO)
# ──────────────────────────────────────────────────────────────────────
...
def soft_clip_v8(x: float) -> float:
    # Compresión asimétrica: solo toca a los que se pasan del techo
...
def calcular_lambdas(df: pd.DataFrame, eq_a: str, eq_b: str, a_es_local: bool, rot_a: float = 0.0, rot_b: float = 0.0):
    ...
    def stats_blended(eq: str, cond: str):
        ...
        gf_blend = gf_spec * w_s + (row_gen["GF"].sum()/pj_gen if pj_gen>0 else LIGA_MEDIA_GOLES) * (1-w_s)
        gc_blend = gc_spec * w_s + (row_gen["GC"].sum()/pj_gen if pj_gen>0 else LIGA_MEDIA_GOLES) * (1-w_s)
    ...
    return round(float(np.clip(lam_a, 0.40, 3.8)), 3), round(float(np.clip(lam_b, 0.40, 3.8)), 3)
...
def fig_radar(df, eq_a, eq_b, cond_a, cond_b):
    ...
    fig.update_layout(**PLOT, height=400, polar=dict(radialaxis=dict(visible=False), bgcolor="rgba(0,0,0,0)"))
    return fig
...
if nav == "🔮 Predictor":
    st.markdown('<div class="section-title">🔮 Predictor Pro V8.2</div>', unsafe_allow_html=True)
    ...
    if st.button("🚀 SIMULAR PARTIDO"):
        ...
        st.markdown(f'<div class="note">⚙️ Parámetros: λ {eq_a}={la} | λ {eq_b}={lb}</div>', unsafe_allow_html=True)
    ...
    if vista == "Barras":
        ...
        d_mat = df[df["Métrica"] == m_sel].groupby("Equipo").agg(P=("Propio", "mean"), C=("Concedido", "mean")).reset_index()
        ...
        st.plotly_chart(fig.update_layout(**PLOT, height=600, xaxis_title="Concedido", yaxis_title="Propio"), use_container_width=True)

elif nav == "🔄 Head-to-Head":
    st.markdown('<div class="section-title">🔄 Head-to-Head</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2); eq1, eq2 = c1.selectbox("Equipo A", equipos), c2.selectbox("Equipo B", equipos, index=1)
    s1, s2 = df[df["Equipo"]==eq1].groupby("Métrica")["Propio"].mean(), df[df["Equipo"]==eq2].groupby("Métrica")["Propio"].mean()
    df_h2h = pd.DataFrame({eq1: s1, eq2: s2}).round(2).dropna()
    st.dataframe(df_h2h, use_container_width=True)
    st.plotly_chart(fig_radar(df, eq1, eq2, "General", "General"), use_container_width=True)
    ...
    if not d_eq.empty:
        ...
        st.plotly_chart(fig.update_layout(**PLOT, barmode="group", height=500), use_container_width=True)
