...
W_XG = 0.85            # Prioridad al xG
K_SHRINK = 5.0         # Suavizado de anomalías
DC_RHO = -0.10
MAX_GOALS_MATRIX = 7
...
def _league_stats(df):
    ...
    # Referencia híbrida: Si la liga promedia 1.5 xG pero 1.2 goles, la referencia es el blend.
...
def _strength(df, eq, cond, league):
    ...
    # Shrinkage K=5.0: Si Barracas tiene poco xG, el modelo lo tira fuerte hacia abajo.
...
if nav == "🔮 Predictor":
    ...
    if st.button("🚀 INICIAR ANÁLISIS"):
        ...
        st.markdown(f'<div class="note">⚙️ Filtro de Realidad: W_xG={W_XG} | λ {ea}:{la} vs λ {eb}:{lb}</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">🔄 Head-to-Head (Favor vs Contra)</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">📖 Historial por Partido</div>', unsafe_allow_html=True)
    ...
    if "Posesión de balón" in metricas:
        ...
        fig = go.Figure(go.Scatter(x=df_e["Posesión"], y=df_e["Creación"], mode="markers+text", text=df_e.index, marker=dict(size=df_e["Concedido"]*5, color=RED, showscale=True)))
